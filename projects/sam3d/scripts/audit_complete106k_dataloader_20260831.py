#!/usr/bin/env python3
"""Audit the frozen complete-condition subset before the 5k distributed run."""
...
INDEX = f"{OBJECTS}/native_complete_106k_frozen_20260831.txt"
BATCHES = [
...
def main() -> None:
    index_entries = [line.strip() for line in Path(INDEX).read_text(encoding="utf-8").splitlines() if line.strip()]
    if len(index_entries) != len(set(index_entries)):
        raise ValueError("Frozen native index contains duplicate samples")
    if len(index_entries) < 100_000:
        raise ValueError(f"Expected roughly 100k complete samples, found only {len(index_entries)}")
    ...
    if len(dataset) < 100_000:
        raise ValueError(f"Expected roughly 100k DataLoader-eligible samples, found only {len(dataset)}")
    ...
    probe_count = min(256, len(dataset))
