...
def test_zero_gates_preserve_cosmos_and_conditions_can_change_output():
    ...
    model = MinimalV1LVGSam3DDiT(
        ...
        # Hydra's unstructured LazyCall may deliver CLI booleans as strings.
    ...
    assert model.sam3d_teacher_tokens_as_condition is False
    teacher_only = dict(condition)
    for key in list(teacher_only):
        if key != "sam3d_tokens_B_F_N_D":
            teacher_only[key] = None
    with torch.no_grad():
        assert model._build_sam_context(output_dtype=torch.bfloat16, **teacher_only) is None
