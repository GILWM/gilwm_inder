...
This preserves the exact SAM 3 masks used to produce each native sidecar.
Teacher tokens can be dropped because the no-RePA training path does not use
them.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--drop-teacher-tokens", action="store_true")
...
def normalized_native_conditions(
    ...
    if valid and not torch.count_nonzero(shape_out[:valid]):
...
def merge_one(
    payload: tuple[str, str, str, str, int, int, bool, bool, bool]
) -> tuple[str, str]:
    (
        base_path_s,
        base_root_s,
    ...
    try:
        ...
        if drop_teacher_tokens:
            base.pop("sam3d_tokens", None)
