...
# MUSA uses PyTorch SDPA; FlashAttention2 is CUDA-specific.
...
class Qwen2_5_VLRotaryEmbedding(nn.Module):
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        raise NotImplementedError("Dynamic frequency update is not implemented for Qwen2_5_VL")
        # """
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # logger.info(
        #     f"hidden_states: {hidden_states.shape}, position_embeddings: {position_embeddings[0].shape}, {position_embeddings[1].shape}"
        # )
        # Self Attention
...
class Qwen2_5_VLModel(nn.Module):
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            layer_id += 1
            if self.gradient_checkpointing and self.training:
                raise NotImplementedError("Gradient checkpointing is not implemented here")
                ...
                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=causal_mask,  # None
                    position_ids=position_ids,
                    past_key_value=past_key_values,  # None
                    output_attentions=output_attentions,  # bool
                    use_cache=use_cache,  # bool
                    cache_position=cache_position,  # None
                    position_embeddings=position_embeddings,
                    cp_mesh=self.cp_mesh,  # device mesh
                )
    ...
    def _update_causal_mask(
        ...
        if (
            ...
            and attention_mask.device.type in ["musa", "xpu"]
            and not output_attentions
        ):
...
class Qwen2_5_VLForConditionalGenerationSimple(nn.Module):
    ...
    def init_weights(self, buffer_device: Optional[torch.device] = None):
        ...
        self.visual.init_weights(buffer_device)
...
__all__ = ["Qwen2_5_VLForConditionalGenerationSimple"]
