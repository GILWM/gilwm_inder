...
try:
    ...
    print("FSDP2 is not supported in this version of PyTorch.")

from cosmos_predict2._src.imaginaire.utils import log as logger


def get_device_info():
    # torch_musa exposes compatibility shims under torch.cuda, which can make
    # PyTorch's private device probe incorrectly report CUDA. Prefer the real
    # MUSA backend whenever it is available.
    if hasattr(torch, "musa") and torch.musa.is_available():
        return "musa", torch.musa
    device_type = _get_available_device_type()
    if device_type is None:
        device_type = "musa"  # default device_type: cuda
    device_module = _get_device_module(device_type)  # default device_module:torch.musa
...
def clip_grad_norm_(
    ...
    if isinstance(parameters, torch.Tensor):
        ...
        parameters = list(parameters)  # prevent generators from being exhausted
    grads = [p.grad for p in parameters if p.grad is not None]
    total_norm = torch.nn.utils.clip_grad._get_total_norm(grads, norm_type, error_if_nonfinite, foreach)
