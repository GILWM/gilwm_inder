...
class Text2WorldModelRectifiedFlow(ImaginaireModel):
    ...
    def __init__(self, config: Text2WorldModelRectifiedFlowConfig):
        ...
        self.tensor_kwargs = {"device": "musa", "dtype": self.precision}
        self.tensor_kwargs_fp32 = {"device": "musa", "dtype": torch.float32}
    ...
    def build_net(self, keep_on_cpu: bool = False):
        ...
        with misc.timer("Creating PyTorch model"):
            ...
            # Materialize inference parameters directly in the configured
            # precision.  Constructing the 14B meta model as FP32 first makes
            # to_empty reserve about 53 GiB per device before the BF16
            # checkpoint is loaded, while the actual BF16 weights need about
            # 29 GiB.  Casting meta tensors is allocation-free and avoids that
            # transient peak on MUSA.
    ...
    def apply_fsdp(self, dp_mesh: DeviceMesh) -> None:
        ...
        # Every rank loaded identical weights from the shared local checkpoint
        # before FSDP sharding, so no initialization broadcast is required.
    ...
    def forward(self, data_batch: dict[str, torch.Tensor]) -> tuple[dict[str, torch.Tensor], torch.Tensor]:
        ...
        if self.config.text_encoder_config is not None and self.config.text_encoder_config.compute_online:
            ...
            data_batch["t5_text_mask"] = torch.ones(text_embeddings.shape[0], text_embeddings.shape[1], device="musa")
