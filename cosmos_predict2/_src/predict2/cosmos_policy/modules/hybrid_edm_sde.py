...
class HybridEDMSDE(EDMSDE):
    ...
    def sample_t(self, batch_size: int) -> torch.Tensor:
        ...
        if self.hybrid_sigma_distribution:
            samples = torch.zeros(batch_size, device="musa")
            ...
            distribution_choice = torch.rand(batch_size, device="musa") < 0.7
            ...
            if num_lognormal > 0:
                ...
                log_sigmas = torch.tensor([self.gaussian_dist.inv_cdf(cdf_val) for cdf_val in cdf_vals], device="musa")
            ...
            if num_uniform > 0:
                ...
                samples[~distribution_choice] = torch.empty(num_uniform, device="musa").uniform_(
