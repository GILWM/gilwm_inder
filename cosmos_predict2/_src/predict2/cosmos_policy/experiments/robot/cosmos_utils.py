...
def get_planning_model(cfg, device: str = "musa"):
    """
        ...
        try:
            with lock:
                with open(t5_text_embeddings_path, "rb") as file:
                    ...
                    device = torch.device(f"musa:{worker_id}" if torch.musa.is_available() else "cpu")
...
def get_action(
    ...
    with torch.inference_mode():
        ...
        if isinstance(task_label_or_embedding, str):
            ...
            text_embedding = torch.tensor(task_label_or_embedding, dtype=torch.bfloat16).to("musa")
        ...
        raw_image_sequence = torch.from_numpy(raw_image_sequence).to(dtype=torch.uint8).to("musa")
        if cfg.use_proprio:
            # Convert proprio to tensor so that it can be injected into latent later
            proprio_tensor = (
                torch.from_numpy(proprio).reshape(batch_size, -1).to(dtype=torch.bfloat16).to("musa")
            )  # (B, proprio_dim)
        ...
        if text_embedding.shape[0] == batch_size:
            text_embedding_batch = text_embedding.to(dtype=torch.bfloat16).to("musa")
        else:
            text_embedding_batch = text_embedding.repeat(batch_size, 1, 1).to(dtype=torch.bfloat16).to("musa")
        ...
        data_batch = {
            ...
            ).to("musa"),  # Just match the training config (always 16 FPS)
            "padding_mask": torch.zeros(
                (batch_size, 1, COSMOS_IMAGE_SIZE, COSMOS_IMAGE_SIZE), dtype=torch.bfloat16
            ).to("musa"),  # Padding mask (assume no padding here)
            ...
            "current_proprio_latent_idx": (
                torch.tensor([current_proprio_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_proprio
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "current_wrist_image_latent_idx": (
                torch.tensor([current_wrist_image_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_wrist_image
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "current_wrist_image2_latent_idx": (
                torch.tensor([current_wrist_image2_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_wrist_image
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "current_image_latent_idx": (
                torch.tensor([current_image_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_third_person_image
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "current_image2_latent_idx": (
                torch.tensor([current_image2_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_third_person_image and cfg.num_third_person_images == 2
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "action_latent_idx": torch.tensor([action_latent_idx] * batch_size, dtype=torch.int64).to("musa"),
            "future_proprio_latent_idx": (
                torch.tensor([future_proprio_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_proprio
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "future_wrist_image_latent_idx": (
                torch.tensor([future_wrist_image_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_wrist_image
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "future_wrist_image2_latent_idx": (
                torch.tensor([future_wrist_image2_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_wrist_image and cfg.num_wrist_images == 2
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "future_image_latent_idx": (
                torch.tensor([future_image_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_third_person_image
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "future_image2_latent_idx": (
                torch.tensor([future_image2_latent_idx] * batch_size, dtype=torch.int64).to("musa")
                if cfg.use_third_person_image and cfg.num_third_person_images == 2
                else torch.tensor([-1] * batch_size, dtype=torch.int64).to("musa")
            ),
            "value_latent_idx": torch.tensor([value_latent_idx] * batch_size, dtype=torch.int64).to("musa"),
        }
