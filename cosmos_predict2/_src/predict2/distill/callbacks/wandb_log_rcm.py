...
class WandbCallback(WandBCallbackBase):
    def __init__(
        ...
        self.img_unstable_count = torch.zeros(1, device="musa")
        self.video_unstable_count = torch.zeros(1, device="musa")
        ...
        if not skip_update_due_to_unstable_loss:
            ...
            if "dmd_loss" in output_batch.keys():
                ...
                if not isinstance(_dmd_loss_value, torch.Tensor):
                    _dmd_loss_value = torch.tensor(_dmd_loss_value, device="musa", dtype=torch.float32)
                else:
