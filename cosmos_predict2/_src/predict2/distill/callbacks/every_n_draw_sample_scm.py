    ...
    def sample_fixed(self, trainer, model, data_batch, output_batch, loss, iteration):
        ...
        for prompt, text_emb in self.kv_prompt_to_emb.items():
            ...
            data_batch["t5_text_mask"] = torch.ones(
                data_batch["t5_text_embeddings"].shape[0], data_batch["t5_text_embeddings"].shape[1], device="musa"
            )
            ...
            del video
            del emb
            torch.musa.synchronize()  # optional but helpful to ensure kernels finished
            torch.musa.empty_cache()
