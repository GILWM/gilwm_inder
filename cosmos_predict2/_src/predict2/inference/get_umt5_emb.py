...
"""
Usage:
    pytest -s projects/cosmos/diffusion/v2/networks/umt5.py --all
TODO:
...
def load_model_dcp(model, ckpt_path, credential_path: Optional[str] = None):
    storage_reader = get_storage_reader(ckpt_path, credential_path)
    _state_dict = get_model_state_dict(model)
    dcp.load(_state_dict, storage_reader=storage_reader, planner=DefaultLoadPlanner(allow_partial_load=True))
    log.info(set_model_state_dict(model, _state_dict, options=StateDictOptions(strict=False)))
    return model
...
class UMT5EncoderModel:
    def __init__(
        ...
        if checkpoint_path.endswith(".dcp"):
...
def get_umt5_embedding(
    ...
    if t5_encoder is None:
        t5_encoder = UMT5EncoderModel(device=device)
    return t5_encoder(prompts, device=device)


def get_negative_emb():
    easy_io.set_s3_backend(
        backend_args={
            "backend": "s3",
