...
_SAM3D_CACHE_FIELDS = {
    "sam3d_tokens": ("sam3d_tokens_B_F_N_D", 4),
    "sam_masks": ("sam_mask_B_K_H_W", 4),
    "sam_mask_meta": ("sam_mask_meta_B_K_D", 3),
    "sam3d_geometry": ("sam3d_geometry_B_C_H_W", 4),
    "sam3d_shape_latents": ("sam3d_shape_latents_B_K_N_D", 4),
    "sam3d_object_pose": ("sam3d_object_pose_B_K_D", 3),
}
...
def load_sam3d_condition_cache(path: str) -> dict[str, torch.Tensor]:
    """Load and validate an offline SAM 3D condition cache for one sample."""

    if not os.path.isfile(path):
        raise FileNotFoundError(f"SAM 3D condition cache does not exist: {path}")
    try:
        cache = torch.load(path, map_location="cpu", weights_only=True)
    except TypeError:
        cache = torch.load(path, map_location="cpu")
    if not isinstance(cache, dict):
        raise TypeError(f"SAM 3D condition cache must be a dictionary: {path}")
    ...
    for source_key, (target_key, expected_ndim) in _SAM3D_CACHE_FIELDS.items():
        ...
        if tensor.ndim != expected_ndim or tensor.shape[0] != 1:
            raise ValueError(
                f"{source_key} in {path} must be a single-sample {expected_ndim}D tensor, "
                f"got {tuple(tensor.shape)}"
            )
        if not torch.isfinite(tensor).all():
            raise ValueError(f"{source_key} in {path} contains NaN or Inf")
        output[target_key] = tensor.contiguous()
        shapes[source_key] = tuple(tensor.shape)

    log.info(f"Loaded SAM 3D inference condition from {path}: {shapes}")
    return output
...
class Video2WorldInference:
    ...
    def __init__(
        ...
        model_device = None if offload_diffusion_model else "musa"
    ...
    def _get_data_batch_input(
        ...
        sam3d_conditions: dict[str, torch.Tensor] | None = None,
    ):
        """
    ...
    def generate_vid2world(
        ...
        sam3d_condition_path: str | None = None,
        guidance: int = 7,
        ...
        # Standard (non-autoregressive) inference must honor the requested output
        # length. The upstream default state_t=24 maps to 93 pixel frames; without
        # updating it, passing num_video_frames=121 silently still generates 93.
        requested_state_t = self.model.tokenizer.get_latent_num_frames(num_video_frames)
        if requested_state_t != self.model.config.state_t:
            log.info(
                f"Updating single-chunk temporal length: state_t "
                f"{self.model.config.state_t} -> {requested_state_t} "
                f"for {num_video_frames} output frames"
            )
            self.model.config.state_t = requested_state_t

        # Get the exact pixel length represented by the latent temporal size.
        model_required_frames = self.model.tokenizer.get_pixel_num_frames(self.model.config.state_t)
        if model_required_frames != num_video_frames:
            raise ValueError(
                f"Requested {num_video_frames} frames cannot be represented exactly by the tokenizer; "
                f"state_t={requested_state_t} maps to {model_required_frames} frames"
            )
        ...
        data_batch = self._get_data_batch_input(
            ...
            use_neg_prompt=True,
            sam3d_conditions=(
                load_sam3d_condition_cache(sam3d_condition_path)
                if sam3d_condition_path is not None
                else None
            ),
        )

        mem_bytes = torch.musa.memory_allocated(device=torch.device("musa" if torch.musa.is_available() else "cpu"))
        ...
        if self.offload_tokenizer:
            ...
            if hasattr(self.model.tokenizer, "encoder") and self.model.tokenizer.encoder is not None:
                self.model.tokenizer.encoder = self.model.tokenizer.encoder.to("musa")
            torch.musa.empty_cache()
        ...
        if self.offload_tokenizer:
            ...
            if hasattr(self.model.tokenizer, "decoder") and self.model.tokenizer.decoder is not None:
                self.model.tokenizer.decoder = self.model.tokenizer.decoder.to("musa")
            torch.musa.empty_cache()
        ...
        if self.offload_text_encoder and self.model.text_encoder is not None:
            ...
            # TextEncoder is a wrapper class with self.model (the actual neural network)
            if hasattr(self.model.text_encoder, "model") and self.model.text_encoder.model is not None:
                self.model.text_encoder.model = self.model.text_encoder.model.to("musa")
            torch.musa.empty_cache()
    ...
    def generate_autoregressive_from_batch(
        ...
        # Autoregressive chunks define the model's native temporal window.  A
        # checkpoint may have been saved with a different training length
        # (for example 121 frames / 31 latent frames), so deriving this from
        # the stale checkpoint config would silently pad a requested 93-frame
        # chunk back to 121 frames.  Cosmos DiT is temporally shape-dynamic;
        # set state_t from the requested chunk and require an exact tokenizer
        # round trip before sampling.
        requested_state_t = self.model.tokenizer.get_latent_num_frames(chunk_size)
        model_required_frames = self.model.tokenizer.get_pixel_num_frames(requested_state_t)
        if model_required_frames != chunk_size:
            raise ValueError(
                f"Autoregressive chunk_size={chunk_size} cannot be represented exactly by the tokenizer; "
                f"state_t={requested_state_t} maps to {model_required_frames} frames"
            )
        if self.model.config.state_t != requested_state_t:
            log.info(
                f"Updating autoregressive temporal window: state_t "
                f"{self.model.config.state_t} -> {requested_state_t} "
        ...
        for chunk_idx in range(num_chunks):
            ...
            chunk_video = self.generate_vid2world(
                ...
                sam3d_condition_path=sam3d_condition_path,
                guidance=guidance,
                num_video_frames=model_required_frames,
