...
def load_model_state_dict_from_checkpoint(
    ...
    if load_from_local:
        # Load independently on every rank from the shared local filesystem.
        # This avoids large initialization broadcasts that are unreliable on
        # the current MUSA/MCCL stack while producing identical model states.
        if True:
        ...
        log.info("All ranks loaded the shared local checkpoint; skipping initialization state broadcast.")
    else:
