...
class CameraARMiniTrainDIT(WeightTrainingStat):
    """
...
class CameraARMiniTrainDITwithConditionalMask(CameraARMiniTrainDIT):
...
def replace_selfattn_op_with_sparse_attn_op(
    model: CameraARMiniTrainDIT, n_dense_blocks: int = 0, natten_parameters: Union[dict, list] = None
) -> CameraARMiniTrainDIT:
    """
    Replace the self-attention operator with a sparse self-attention operator.

    Args:
        model: CameraARMiniTrainDIT instance
