...
class TimestepEmbedding(nn.Module):
    ...
    def forward(self, sample: torch.Tensor) -> torch.Tensor:
        # muDNN does not support FP32 activations multiplied by BF16 weights.
        # Match the activation dtype to each linear layer explicitly.
        emb = self.linear_1(sample.to(dtype=self.linear_1.weight.dtype))
        emb = self.activation(emb)
        emb = self.linear_2(emb.to(dtype=self.linear_2.weight.dtype))
