...
class ModelWrapper(Stateful):
    ...
    def state_dict(self, mapping_keys: dict[str, str] = {}) -> Dict[str, Any]:
        get_state = functools.partial(
            get_model_state_dict,
            options=StateDictOptions(cpu_offload=self.cpu_offload),
        )
        _state_dict = {k: v for sd in map(get_state, self.model) for k, v in sd.items()}
        if self.load_ema_to_reg:
...
class DistributedCheckpointer(AbstractCheckpointer):
    ...
    def __init__(
        ...
        if self.async_mode == AsyncMode.ASYNC_WITH_PINNED_MEM:
            ...
            self.staging_stream = torch.musa.Stream()
    ...
    def load(
        ...
        if checkpoint_path is not None:
            ...
            # Optional hybrid initialization for SAM3D training.  The primary
            # checkpoint is loaded first (normally a trained SAM3D checkpoint),
            # then every model key present in the overlay checkpoint replaces
            # the matching value.  Because a plain Cosmos checkpoint has no
            # SAM3D-specific keys, those parameters remain from the primary
            # checkpoint while the Cosmos backbone (including EMA) is replaced.
            cosmos_overlay_checkpoint = os.environ.get("SAM3D_COSMOS_OVERLAY_CHECKPOINT", "").strip()
            if cosmos_overlay_checkpoint:
                overlay_model_path = cosmos_overlay_checkpoint.rstrip("/")
                if not overlay_model_path.endswith("/model"):
                    overlay_model_path = os.path.join(overlay_model_path, "model")
                log.critical(
                    "Hybrid initialization: overlaying matching Cosmos model keys "
                    f"from {cosmos_overlay_checkpoint}; unmatched SAM3D keys stay from {checkpoint_path}"
                )
                overlay_storage_reader = self.get_storage_reader(overlay_model_path)
                torch.distributed.barrier()
                overlay_model_wrapper = ModelWrapper(model)
                overlay_state_dict = overlay_model_wrapper.state_dict()
                dcp_load_state_dict(
                    overlay_state_dict,
                    overlay_storage_reader,
                    DefaultLoadPlanner(allow_partial_load=True),
                )
                overlay_model_wrapper.load_state_dict(overlay_state_dict)
                torch.distributed.barrier()
                log.critical(
                    "Hybrid initialization complete: Cosmos model keys came from "
                    f"{cosmos_overlay_checkpoint}; SAM3D-only keys came from {checkpoint_path}"
                )
            if self.callbacks is not None:
    ...
    def _async_with_pinned_memory(self, checkpoint_file: str, state_dict: Dict[str, Tuple[Any, str]]) -> None:
        ...
        with torch.musa.stream(self.staging_stream):
    ...
    def save(
        ...
        to_save_dict = {
            "model": ModelWrapper(model, cpu_offload=True).state_dict(),
            "optim": OptimizerWrapper(model, optimizer, cpu_offload=True).state_dict(),
