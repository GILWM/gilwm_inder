...
from cosmos_predict2._src.imaginaire.utils.distributed import get_rank, sync_model_states
...
class WanVAE:
    def __init__(
        self,
        z_dim=48,
        vae_pth="s3://bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt2/Wan2.2_VAE.pth",
        s3_credential_path: str = "credentials/s3_training.secret",
        ...
        if not is_amp:
            ...
            self.context = torch.amp.autocast("musa", dtype=dtype)
...
class Wan2pt2VAEInterface(VideoTokenizerInterface):
    def __init__(self, chunk_duration: int = 93, **kwargs):
        self.model = WanVAE(
            dtype=torch.bfloat16,
            is_amp=False,
            vae_pth=kwargs.get(
                "vae_pth",
                "s3://bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt2/Wan2.2_VAE.pth",
            ),
    ...
    def spatial_compression_factor(self):
        return 16  # 2x from patchify + 8x from spatial downsampling
