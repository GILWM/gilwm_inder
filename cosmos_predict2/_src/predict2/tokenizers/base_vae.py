...
from cosmos_predict2._src.imaginaire.utils.distributed import rank0_first
    ...
    def register_mean_std(self, mean_std_fp: str) -> None:
        ...
        if self.load_mean_std:
            ...
            latent_mean, latent_std = load_from_s3_with_cache(
                ...
                easy_io_kwargs={"map_location": torch.device(torch.musa.current_device())},
            ...
            device = torch.device(torch.musa.current_device()) if torch.musa.is_available() else torch.device("cpu")
            self.register_buffer(
                "latent_mean",
