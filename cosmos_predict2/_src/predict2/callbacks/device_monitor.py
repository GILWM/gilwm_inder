...
class DeviceMonitor(EveryN):
    ...
    def on_train_start(self, model, iteration=0):
        ...
        try:
            ...
            # Moore Threads devices do not expose NVIDIA NVML. The remaining
            # metrics below are provided by torch.musa, so keep monitoring
            # enabled and derive memory figures from the MUSA allocator.
            self.handle = None
    ...
    def every_n_impl(
        ...
        peak_gpu_mem_gb = torch.musa.max_memory_allocated() / (1024**3)
        peak_gpu_mem_reserved_gb = torch.musa.max_memory_reserved() / (1024**3)
        temp = torch.musa.temperature()
        try:
            power = torch.musa.power_draw()
        except Exception as e:
            log.warning(f"Failed to get power draw with error {e}")
            power = 0
        util = torch.musa.utilization()
        clock = torch.musa.clock_rate()
        ...
        if self.handle is not None:
            ...
            nvml_used_gpu_mem_gb = torch.musa.memory_reserved() / (1024**3)
            total_mem = torch.musa.get_device_properties(torch.musa.current_device()).total_memory
            nvml_free_gpu_mem_gb = (total_mem - torch.musa.memory_reserved()) / (1024**3)
        ...
        if self.rank == 0:
            ...
            if self.log_memory_detail:
                memory_stats = torch.musa.memory_stats()
                if wandb.run:
