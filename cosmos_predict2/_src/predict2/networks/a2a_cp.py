...
class _SeqAllToAllQKV(torch.autograd.Function):
    ...
    def forward(
        ...
        if q.device.type == "musa":
            # MCCL 2.11.4 can execute these collectives reliably, but fails
            # after many layers when Q/K/V all-to-all operations overlap on a
            # secondary stream. Keep the equivalent MUSA path synchronous.
            q = single_all_to_all(q, local_seq_2_local_head, group, False)
            k = single_all_to_all(k, local_seq_2_local_head, group, False)
            v = single_all_to_all(v, local_seq_2_local_head, group, False)
        else:
