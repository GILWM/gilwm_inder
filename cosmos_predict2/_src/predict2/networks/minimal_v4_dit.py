...
class TimestepEmbedding(nn.Module):
    ...
    def forward(self, sample: torch.Tensor) -> torch.Tensor:
        # CUDA autocast normally aligns this explicit float32 timestep
        # embedding with the bf16 model weights. torch_musa autocast does not
        # currently do so for this Linear path, and MUDNN rejects mixed input
        # and weight dtypes.
        ...
        # Wan's fp32 strategy asserts that the timestep outputs are float32 at
        # the final layer even though their two Linear operations use bf16.
        emb_B_T_D = emb_B_T_D.float()
        if adaln_lora_B_T_3D is not None:
            adaln_lora_B_T_3D = adaln_lora_B_T_3D.float()
...
class FinalLayer(nn.Module):
    ...
    def forward(
        ...
        # On MUSA the upstream embedding can remain BF16 even when the WAN
        # FP32 strategy is selected.  The modulation input is explicitly cast
        # to its parameter dtype below, so requiring FP32 here is unnecessary.
        adaln_input = emb_B_T_D.to(dtype=self.adaln_modulation[1].weight.dtype)
        with amp.autocast("musa", enabled=self.use_wan_fp32_strategy, dtype=torch.float32):
            if self.use_adaln_lora:
                assert adaln_lora_B_T_3D is not None
                shift_B_T_D, scale_B_T_D = (
                    self.adaln_modulation(adaln_input) + adaln_lora_B_T_3D[:, :, : 2 * self.hidden_size]
                ).chunk(2, dim=-1)
            else:
                shift_B_T_D, scale_B_T_D = self.adaln_modulation(adaln_input).chunk(2, dim=-1)
            ...
            x_B_T_H_W_O = self.linear(
                x_B_T_H_W_D.to(dtype=self.linear.weight.dtype)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        sam_context_gate: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        if extra_per_block_pos_emb is not None:
            x_B_T_H_W_D = x_B_T_H_W_D + extra_per_block_pos_emb

        adaln_input = emb_B_T_D.to(dtype=self.adaln_modulation_self_attn[1].weight.dtype)
        with amp.autocast("musa", enabled=self.use_wan_fp32_strategy, dtype=torch.float32):
            if self.use_adaln_lora:
                shift_self_attn_B_T_D, scale_self_attn_B_T_D, gate_self_attn_B_T_D = (
                    self.adaln_modulation_self_attn(adaln_input) + adaln_lora_B_T_3D
                ).chunk(3, dim=-1)
                shift_cross_attn_B_T_D, scale_cross_attn_B_T_D, gate_cross_attn_B_T_D = (
                    self.adaln_modulation_cross_attn(adaln_input) + adaln_lora_B_T_3D
                ).chunk(3, dim=-1)
                shift_mlp_B_T_D, scale_mlp_B_T_D, gate_mlp_B_T_D = (
                    self.adaln_modulation_mlp(adaln_input) + adaln_lora_B_T_3D
                ).chunk(3, dim=-1)
            else:
                ...
                shift_cross_attn_B_T_D, scale_cross_attn_B_T_D, gate_cross_attn_B_T_D = (
                    self.adaln_modulation_cross_attn(adaln_input).chunk(3, dim=-1)
                )
                shift_mlp_B_T_D, scale_mlp_B_T_D, gate_mlp_B_T_D = self.adaln_modulation_mlp(adaln_input).chunk(3, dim=-1)
        ...
        # A separate SAM/SAM3D context branch preserves the full multi-token
        # spatial/object structure instead of squeezing it into Reason1's one
        # text token.  It intentionally reuses the pretrained cross-attention
        # projections; the per-block scalar gate is zero at initialization, so
        # old Cosmos checkpoints produce bit-identical outputs before training.
        if sam_context_emb is not None and sam_context_gate is not None:
            normalized_sam_x_B_T_H_W_D = _fn(
                x_B_T_H_W_D,
                self.layer_norm_cross_attn,
                scale_cross_attn_B_T_1_1_D,
                shift_cross_attn_B_T_1_1_D,
            )
            sam_result_B_T_H_W_D = rearrange(
                self.cross_attn(
                    rearrange(normalized_sam_x_B_T_H_W_D, "b t h w d -> b (t h w) d"),
                    sam_context_emb,
                    rope_emb=rope_emb_L_1_1_D,
                ),
                "b (t h w) d -> b t h w d",
            ...
            sam_gate = torch.tanh(sam_context_gate).to(dtype=x_B_T_H_W_D.dtype)
            x_B_T_H_W_D = (
                x_B_T_H_W_D
                + sam_gate * gate_cross_attn_B_T_1_1_D * sam_result_B_T_H_W_D
            )
...
class MiniTrainDIT(WeightTrainingStat):
    ...
    def forward(
        ...
        if self.use_crossattn_projection and not crossattn_already_projected:
        ...
        with amp.autocast("musa", enabled=self.use_wan_fp32_strategy, dtype=torch.float32):
            ...
            if self.use_wan_fp32_strategy:
                ...
                # Wan's downstream final layer requires the normalized
                # timestep embedding to remain fp32.
        ...
        for i, block in enumerate(self.blocks):
            sam_context_gate = None
            if sam_context_block_gates is not None:
                if sam_context_block_gates.numel() != len(self.blocks):
                    raise ValueError(
                        "sam_context_block_gates must have one value per DiT block, "
                        f"got {sam_context_block_gates.numel()} for {len(self.blocks)} blocks"
                    )
                # Keep a one-dimensional slice: FSDP2 does not support scalar
                # (zero-dimensional) parameters/tensor views.
                sam_context_gate = sam_context_block_gates[i : i + 1]
            x_B_T_H_W_D = block(
                x_B_T_H_W_D,
                t_embedding_B_T_D,
                context_input,
                ...
                sam_context_gate=sam_context_gate,
            )
