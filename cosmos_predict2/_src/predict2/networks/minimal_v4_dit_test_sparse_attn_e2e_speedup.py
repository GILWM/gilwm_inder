...
def test_inference_e2e():
    ...
    for net_str, net_cfg, n_dense_blocks, natten_parameters in [
        ...
        for T in T_list:
            ...
            video_example_input = {
                "x_B_C_T_H_W": torch.randn(batch_size, 16, T_, H_, W_, dtype=torch.bfloat16, device="musa"),
                "timesteps_B_T": torch.randn(batch_size, dtype=torch.bfloat16, device="musa"),
                "crossattn_emb": torch.randn(batch_size, 512, 1024, dtype=torch.bfloat16, device="musa"),
                "fps": torch.randint(size=(batch_size,), low=2, high=30, device="musa"),
                "padding_mask": torch.randn(batch_size, 1, H_, W_, dtype=torch.bfloat16, device="musa"),
