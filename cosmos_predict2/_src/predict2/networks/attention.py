...
def get_device_cc(device) -> int:
    ...
    """
    if torch.musa.is_available() and torch.version.cuda and device.type == "musa":
...
def attention(
    ...
    if compute_cap == 90 and FLASH_ATTN_3_AVAILABLE and is_half:
        ...
        if q.device.type == "musa":
            # Do not pass CUDA-only backend enums to torch_musa. Its default
            # dispatcher selects the native memory-efficient implementation;
            # forcing MATH materializes the full long-sequence attention matrix.
