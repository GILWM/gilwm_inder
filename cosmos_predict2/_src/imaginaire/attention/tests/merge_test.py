...
class AttentionMergeTest(unittest.TestCase):
    ...
    def _test(
        ...
        for dtype, atol_out, (atol_dq, atol_dk, atol_dv) in ALLOWED_DTYPES:
            ...
            q = torch.randn(batch, seqlen_Q, heads, head_dim, device="musa", dtype=dtype)
            k_0 = torch.randn(batch, seqlen_KV_0, heads_kv, head_dim, device="musa", dtype=dtype)
            v_0 = torch.randn(batch, seqlen_KV_0, heads_kv, head_dim_v, device="musa", dtype=dtype)
            k_1 = torch.randn(batch, seqlen_KV_1, heads_kv, head_dim, device="musa", dtype=dtype)
            v_1 = torch.randn(batch, seqlen_KV_1, heads_kv, head_dim_v, device="musa", dtype=dtype)
            do = torch.randn(batch, seqlen_Q, heads, head_dim_v, device="musa", dtype=dtype)
