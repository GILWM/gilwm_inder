...
def compute_split_reference(
    ...
    with torch.no_grad():
        q_ref, k_ref, v_ref, d_out_ref = (
            torch.randn((1, seqlen_q_total, heads, head_dim), device="musa", dtype=dtype_safe).to(dtype),
            torch.randn(
            ...
            torch.randn((1, seqlen_q_total, heads, head_dim_v), device="musa", dtype=dtype_safe).to(dtype),
        )
        q, k, v, d_out = (
            q_ref.clone(),
            k_ref.clone(),
