...
from cosmos_predict2._src.imaginaire.auxiliary.guardrail.common.io_utils import (
    get_video_filepaths,
    read_video,
    save_video,
)
from cosmos_predict2._src.imaginaire.auxiliary.guardrail.face_blur_filter.blur_utils import pixelate_face
from cosmos_predict2._src.imaginaire.auxiliary.guardrail.face_blur_filter.retinaface_utils import (
    decode_batch,
...
class RetinaFaceFilter(PostprocessingGuardrail):
    def __init__(
        ...
        if not offload_model_to_cpu:
            ...
            self.net.to("musa", dtype=self.dtype).eval()
    ...
    def preprocess_frames(self, frames: np.ndarray) -> torch.Tensor:
        ...
        """
        with torch.no_grad():
            frames_tensor = torch.from_numpy(frames).to("musa", dtype=self.dtype)  # Shape: [T, H, W, C]
            ...
            means = torch.tensor([104.0, 117.0, 123.0], device="musa", dtype=self.dtype).view(1, 3, 1, 1)
        ...
        for i in range(0, num_frames, self.batch_size):
            ...
            with torch.no_grad():
                ...
                if prior_data is None:
                    ...
                    priors = priors.to("musa", dtype=self.dtype)
                    prior_data = priors.data

                # Get scale for resizing detections
                if scale is None:
                    scale = torch.Tensor([w, h, w, h])
                    scale = scale.to("musa", dtype=self.dtype)
