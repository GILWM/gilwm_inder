...
class VideoContentSafetyFilter(ContentSafetyGuardrail):
    def __init__(
        ...
        self.encoder = SigLIPEncoder(device="musa", dtype=self.dtype)
        if offload_model_to_cpu:
            ...
            self.encoder.to("musa")
            self.model = self.model.to("musa", dtype=self.dtype).eval()
    ...
    def _to_cuda_if_offload(self):
        if self.offload_model:
            self.encoder = self.encoder.to("musa")
            self.model = self.model.to("musa")
