...
"""
Forked from https://github.com/openai/CLIP/blob/main/clip/model.py
This file differs in that it exposes the prepooled patch tokens alongside the global image tokens
when calling the CLIP model
"""
...
class AttentionPool2d(nn.Module):
    """Attention pooling layer for 2D feature maps."""
...
class ModifiedResNet(nn.Module):
    ...
    def _make_layer(self, planes, blocks, stride=1):
        ...
        - planes: the number of output channels for this layer
        - blocks: the number of residual blocks in this layer
        - stride: the stride to use for the first convolution of the layer
        """
    ...
    def forward(self, x):
        def stem(x):
            """
            The stem convolutions at the beginning of the network.
            Performs 3 convolutions and an average pool.
            """
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x, x_pre_pooling


class CLIP(nn.Module):
    """
    This CLIP module combines a visual encoder and a text encoder. It initializes the parameters, builds the attention mask,
    and provides methods to encode images and text separately. The forward method computes the cosine similarity between
    the encoded image and text features.
    """
    ...
    def initialize_parameters(self):
        """Initialize the parameters of the CLIP module."""
    ...
    def encode_text(self, text):
        ...
        Args:
            text (torch.Tensor): The input text.
    ...
    def forward(self, image, text):
        """
        Forward pass of the CLIP module.
        ...
        Returns:
            tuple: A tuple containing the logits per image and logits per text.
        """
...
def build_model(state_dict: dict):
    """Build the CLIP model from a state dictionary."""
    vit = state_dict.get("visual.proj") is not None
...
def _transform(n_px):
    ...
    Args:
        n_px (int): The desired size of the transformed image.
...
def load(
    name: str,
    device: Union[str, torch.device] = "musa" if torch.musa.is_available() else "cpu",
    jit: bool = False,
    ...
    def patch_device(module):
        ...
        for graph in graphs:
            for node in graph.findAllNodes("prim::Constant"):
                if "value" in node.attributeNames() and str(node["value"]).startswith("musa"):
