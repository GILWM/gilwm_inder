...
from cosmos_predict2._src.imaginaire.utils.device import Device
...
if TYPE_CHECKING:
    from cosmos_predict2._src.imaginaire.config import DDPConfig
...
def init() -> int | None:
    ...
    is_musa = hasattr(torch, "musa") and torch.musa.is_available()
    # NVML and the NVIDIA affinity helper are unavailable on MUSA systems.
    if not is_musa:
        try:
            pynvml.nvmlInit()
    ...
    if dist.is_available():
        ...
        dist.init_process_group(backend="mccl", init_method="env://", timeout=timeout_timedelta)
        log.critical(
            f"Initialized distributed training with local rank {local_rank} with timeout {timeout_seconds}",
            rank0_only=False,
        )
    # This CUDA runtime tuning has no MUSA equivalent and is optional.
    if not is_musa:
...
def is_local_rank0() -> bool:
    ...
    """
    return torch.musa.current_device() == 0
...
def collate_batches(data_batches: list[dict[str, torch.Tensor]]) -> torch.Tensor | dict[str, torch.Tensor]:
    ...
    if isinstance(data_batches[0], torch.Tensor):
        ...
        max_num_local_samples = torch.tensor(len(data_concat), device="musa")
        ...
        if len(data_concat) < max_num_local_samples:
            ...
            dummy_count = torch.tensor(1, device="musa")
        else:
            dummy_count = torch.tensor(0, device="musa")
