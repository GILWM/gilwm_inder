...
def _make_graphed_callables(
    ...
    fwd_graphs = [torch.musa.CUDAGraph() for _ in range(len(flatten_sample_args))]
...
def make_graphed_callables_forward(
    ...
    `original PyTorch implementation <https://pytorch.org/docs/stable/generated/torch.musa.make_graphed_callables.html>`_
    ...
    pool: (tuple of) int, default = `None`, optional
          An instance returned from function `torch.musa.graph_pool_handle` that hints
    ...
    if graph_safe_rng_available():
        generators = [
            torch.musa.default_generators[torch.musa.current_device()],
            *get_all_rng_states().values(),
        ]
        original_rng_states = [state.get_state() for state in generators]
    else:
        original_rng_states = torch.musa.get_rng_state()
    ...
    if graph_safe_rng_available():
        ...
        torch.musa.set_rng_state(original_rng_states)
    set_capture_end()
