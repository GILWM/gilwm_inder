...
class CheckpointFileHf(_CheckpointHf):
    ...
    @override
    def _download(self) -> str:
        """Return a local mirror first, otherwise download from Hugging Face."""
        local_root = os.environ.get("COSMOS_LOCAL_MODEL_ROOT")
        if local_root:
            local_path = os.path.join(local_root, self.repository, self.filename)
...
class CheckpointDirHf(_CheckpointHf):
    ...
    def _download(self) -> str:
        ...
        local_root = os.environ.get("COSMOS_LOCAL_MODEL_ROOT")
        if local_root:
            local_path = os.path.join(local_root, self.repository, self.subdirectory)
