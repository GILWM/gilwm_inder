...
class Timer(ContextDecorator):
    ...
    def start(self, cuda_device: torch.device | None = None, cuda_stream: torch.musa.Stream | None = None):
        """
        ...
        Args:
            ...
            cuda_stream (torch.musa.Stream | None): CUDA stream to use for CUDA time measurement.
        ...
        if self.measure_cuda:
            self.cuda_stream = cuda_stream if cuda_stream is not None else torch.musa.current_stream(cuda_device)
        ...
        if self.measure_cuda:
            self.cuda_start_event = torch.musa.Event(enable_timing=True)
            self.cuda_end_event = torch.musa.Event(enable_timing=True)
