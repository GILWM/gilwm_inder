...
class WandBCallback(Callback):
    ...
    def on_validation_start(
        ...
        self._val_cache: dict[str, Any] = dict(
            data_batches=[],
            output_batches=[],
            loss=torch.tensor(0.0, device="musa"),
            sample_size=torch.tensor(0, device="musa"),
        )
...
class NVTXCallback(Callback):
    ...
    def on_before_forward(self, iteration: int = 0) -> None:
        ...
        torch.musa.nvtx.range_push("forward")
    ...
    def on_before_backward(
        ...
        if self.synchronize:
            torch.musa.synchronize()
        torch.musa.nvtx.range_push("backward")
    ...
    def on_before_optimizer_step(
        ...
        if self.synchronize:
            torch.musa.synchronize()
        torch.musa.nvtx.range_push("optimizer_step")
    ...
    def on_before_dataloading(self, iteration: int = 0) -> None:
        torch.musa.nvtx.range_push("dataloading")
