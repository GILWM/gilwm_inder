...
from cosmos_predict2._src.imaginaire.utils.distributed import all_gather_tensor
from cosmos_predict2._src.imaginaire.utils.easy_io import easy_io
from cosmos_predict2._src.imaginaire.utils.timer import Timer
...
class memory_checker(ContextDecorator):  # noqa: N801
    ...
    def __enter__(self) -> None:
        ...
        self.initial_memory = torch.musa.max_memory_allocated()

    def __exit__(self, exc_type, exc_value, traceback) -> None:  # noqa: ANN001
        torch.musa.synchronize()
        final_memory = torch.musa.max_memory_allocated()
    ...
    def __call__(self, func: T) -> T:
        ...
        def wrapper(*args, **kwargs):  # noqa: ANN202
            torch.musa.synchronize()
            torch.musa.reset_peak_memory_stats()
            initial_memory = torch.musa.max_memory_allocated()
...
class NVTXRangeContext:
    """
    Context manager which inserts NVTX range around the current context and optionally calls torch.musa.synchronize
    ...
    Args:
        ...
        synchronize (bool): Whether to call torch.musa.synchronize() at the start and the end of the context. Default: True.
    """
    ...
    def __enter__(self):
        ...
        if self.synchronize:
            torch.musa.synchronize()
        torch.musa.nvtx.range_push(self.name)
    ...
    def generate_report(self, iteration):
        if self.enabled and iteration % self.report_freq == 0:
            ...
            if distributed.get_rank() == 0:
                ...
                import cosmos_predict2._src.imaginaire.utils.launch

                if cosmos_predict2._src.imaginaire.utils.launch.S3_READY and (iteration % (5 * self.report_freq) == 0):
                    easy_io.dump(
                        wandb_info,
