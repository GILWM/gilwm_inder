...
class ModelWrapper(Stateful):
    ...
    def __init__(self, model: Union[nn.Module, List[nn.Module]], load_ema_to_reg: bool = False):
        ...
        if self.load_ema_to_reg:
            from cosmos_predict2._src.interactive.methods.cosmos2_interactive_model import (
                Cosmos2InteractiveModel as cosmos2_interactive_model,
            )
            ...
            assert (
                isinstance(model, predict2_DiffusionModel)
                or isinstance(model, wan2pt1_DiffusionModel)
                or isinstance(model, predict2_DiffusionModel_rectified_flow)
                or isinstance(model, cosmos2_interactive_model)
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        if self.load_ema_to_reg:
            ...
            assert all(
                k.startswith("net.") or k.startswith("net_fake_score.") or k.startswith("net_discriminator_head.")
                for k in all_keys
            ), "All keys must start with net."
            for k in all_keys:
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        if self.load_ema_to_reg:
            ...
            assert all(
                k.startswith("net_ema.") or k.startswith("net_fake_score.") or k.startswith("net_discriminator_head.")
                for k in all_keys
