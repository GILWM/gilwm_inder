...
class ImaginaireDistillationTrainer:
    """The trained used by the distillation / interactive model training.

    Key differences from the base ImaginaireTrainer:
    ...
    def train(
        ...
        model = model.to("musa", memory_format=self.config.trainer.memory_format)  # type: ignore
        ...
        grad_scaler = torch.amp.GradScaler("musa", **self.config.trainer.grad_scaler_args)
        ...
        with (
            ...
            while True:
                ...
                while True:
                    ...
                    data_batch = misc.to(data_batch, device="musa")
        ...
        with distributed.ddp_sync_grad(model_ddp, grad_accum_iter == self.config.trainer.grad_accum_iter - 1):
            ...
            with self.training_timer("forward"):
                with self.straggler_detector.profile_section(
                    ...
                    output_batch, loss = model_ddp.single_train_step(data, iteration)
