...
class SigmaLossAnalysisPerFrame(Callback):
    ...
    def _gather_and_save(self, cache: _SigmaLossCache, iteration: int, prefix: str, log_viz: bool = True) -> dict:
        ...
        if world_size > 1:
            # Gather sizes first
            local_size = torch.tensor([len(local_sigma)], dtype=torch.long, device="musa")
            ...
            if max_size > 0:
                ...
                padded_sigma = torch.zeros(max_size, num_frames or 1, dtype=torch.bfloat16, device="musa")
                padded_loss = torch.zeros(max_size, num_frames or 1, dtype=torch.bfloat16, device="musa")
