...
class EveryNDrawSampleMultiviewVideo(EveryNDrawSample):
    ...
    def sample(self, trainer, model, data_batch, output_batch, loss, iteration):
        ...
        if torch.musa.is_available():
            mem_allocated_before = torch.musa.memory_allocated() / 1e9
            mem_reserved_before = torch.musa.memory_reserved() / 1e9
            print(
            ...
            mem_allocated_after = torch.musa.memory_allocated() / 1e9
            mem_reserved_after = torch.musa.memory_reserved() / 1e9
            print(
